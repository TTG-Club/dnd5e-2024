<script setup lang="ts">
   ...
   *
   * Хранится ОДНОЙ формулой: число, `@prof`, `@level`, `@classLevel`,
   * `@mod.<abbr>` — ту же грамматику понимают ресурсы черт, активные эффекты и
   * лист. Поле лишь раскладывает формулу на понятный выбор: знать про
   * `@mod.cha` ни автор черты, ни игрок не обязаны. Формулу, написанную руками
   * и не разобравшуюся, поле показывает как есть — подставив вместо неё число,
   * оно потеряло бы написанное.
   */
  ...
  const kindOptions: { value: CounterMaxKind; label: string }[] = [
    ...
    {
      value: 'classLevel',
      label: COUNTER_RESOURCE_LABELS.sourceClassLevel,
    },
