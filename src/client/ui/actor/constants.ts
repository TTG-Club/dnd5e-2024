...
/**
 * Псевдо-пак «все компендиумы» в левой колонке окна выбора — выбран по
 * умолчанию. Общий на все такие окна: строку списка они делят одну, и ключ
 * выбранного пака в ней означает одно и то же.
 */
...
export const COMPENDIUM_PICKER_LABELS = {
  ...
  taken: 'Уже взят',
  /** Кнопка карточки записи в строке списка */
  openDetail: 'Открыть карточку',
...
/**
 * Строка списка в компендиуме и окнах выбора: своей плашки нет — строки и так
 * разделены линией, а плашка вместе с промежутками превращала список в лесенку
 * из таблеток.
 */
 ...
 */
export const LIST_ROW_CARD_CLASS =
  'rounded-lg bg-elevated/30 px-3 hover:bg-accented/40';

/**
 * Строка черты на листе персонажа: выше и темнее остальных — с ней там работают
 * руками, и попасть по ней надо с первого раза.
 */
export const LIST_ROW_SHEET_FEAT_CLASS =
  'min-h-11 rounded-lg bg-accented/30 px-3 hover:bg-accented/50';

/**
 * Плашка строки с рамкой по наведению — у вида и класса: их строки крупнее
 * остальных, и рамка отделяет наведённую от соседних.
 */
export const LIST_ROW_CARD_BORDERED_CLASS =
  'rounded-lg bg-elevated/30 px-3 hover:border-default/50 hover:bg-accented/40';
...
/**
 * Окно выбора значений дара (`GrantPoolPickerModal`). Отдельно от
 * `REF_PICKER_LABELS`: то окно берёт записи компендиума, это — справочники
 * правил, и общего у них только вид.
 */
export const POOL_PICKER_LABELS = {
  /** Строка левой колонки: значения всех видов строки разом */
  allGroups: 'Все значения',
  /** Отметить всё показанное — «все простые» одним движением */
  selectVisible: 'Отметить всё',
  /** Снять отметки со всего показанного */
  clearVisible: 'Снять',
  /** Кнопка, открывающая окно, — рядом с отмеченными значениями */
  open: 'Выбрать',
  /** Отмеченных значений нет */
...
export const REF_PICKER_LABELS = {
  ...
  /**
   * Подтверждение одиночного выбора. От «Добавить» отличается по делу: там
   * запись пополняет список, здесь — занимает единственное поле.
   */
  pick: 'Выбрать',
  /** Плашка фильтра, когда вызывающий не назвал, что именно фильтруют */
  filterPlaceholder: 'Фильтр',
  /** Снять все отметки фильтра */
  filterReset: 'Сбросить фильтр',
  /** Подписи фильтра по типам записей */
  filterFeatCategory: 'Категория',
  filterSpellLevel: 'Круг',
} as const;
 ...
 * запись строкой сами. Буква одна с движком: тот печатает ею формулу хитов
...
/**
 * Приставка источника у заклинания, доступного сверх списка класса: расширение
 * списка от черты, вида или умения класса.
 */
export const PINNED_SPELL_FEATURE_PREFIX = 'Сверх списка класса: ';
...
/**
 * Разделитель списка основных характеристик класса: «Сила И Телосложение» либо
 * «Сила ИЛИ Ловкость». Тем же словом он подписан в мастерской сайта.
 */
export const ABILITY_DELIMITER_LABELS: Record<AbilityDelimiter, string> = {
  and: 'и',
  or: 'или',
};

/** Разделители основных характеристик для выбора в UI-селектах */
export const ABILITY_DELIMITER_OPTIONS: ReadonlyArray<{
...
export const SPECIES_FORM_LABELS = {
  ...
  parent: 'Основной вид',
  parentHint:
    'Укажите, если запись — происхождение (подвид): она перестанет предлагаться '
    + 'самостоятельным видом и появится вариантом после выбора основного.',
  parentNone: 'Самостоятельный вид',
  /** Кнопка, открывающая окно выбора основного вида */
  parentPick: 'Выбрать основной вид',
  /** Заголовок окна выбора: свой, а не общий «Выбор вида» из REF_PICKER_TITLES */
  parentPickTitle: 'Выбор основного вида',
  /** Снять основной вид — запись снова самостоятельная */
  parentClear: 'Сделать самостоятельным',
  featureGrantsTitle: 'Дары особенности',
  featureGrantsHint:
    'Что особенность выдаёт сама: владения, наборы «на выбор» — как у черты.',
  featureModifiersTitle: 'Правки листа',
  featureModifiersHint:
    'Постоянные правки листа от особенности: чувства, скорости, хиты, КД, защиты.',
  recordGrantsTitle: 'Дары записи',
  recordGrantsHint:
    'То, что даёт выбор вида или происхождения целиком. Дары конкретной '
    + 'особенности задаются у самой особенности на вкладке «Особенности».',
  recordModifiersTitle: 'Правки листа записи',
  recordCountersTitle: 'Счётчики ресурсов',
  legacyChoicesBadgePrefix: 'Легаси-варианты: ',
  legacyChoicesHint:
    'У особенности есть встроенные варианты старого формата. Они продолжают '
    + 'работать и сохраняются как есть; новый подвид создаётся отдельной '
    + 'записью вида с указанием основного вида.',
  tabEffects: 'Эффекты',
  effectsHint:
    'Активные эффекты (бонусы к характеристикам, КД, флаги и т.п.). '
    + 'Переносятся на персонажа при выборе вида.',
  featureEffectsHint:
    'Эффекты этого умения. Переносятся на персонажа, когда умение начинает '
    + 'действовать: с первого уровня либо с уровня умения.',
  /**
   * Вкладка с телесными характеристиками вида: тип существа, размер, зрение,
   * скорости. Названа как на сайте (`SPECIES_EDITOR_TABS.properties`) — там она
   * собирает ровно те же поля, и разные подписи у одной и той же вкладки
   * заставляли бы искать размер то в одном месте, то в другом.
   */
  tabProperties: 'Характеристики',
  tabGrants: 'Дары',
  /**
  ...
  /** Заголовок блока «тип существа + размер» на вкладке «Характеристики» */
  bodyTitle: 'Тип и размер',
  ...
  sizesHelp:
    ...
    + 'персонажу.',
  /** Заголовок списка строк роста — по строке на каждый выбранный размер */
  heightsTitle: 'Рост по размерам',
  /**
   * Пояснение к росту: величина справочная. Без этой оговорки поле читается
   * как механика, и от него ждут влияния на дальность и вместимость клетки.
   */
  heightsHint:
    'Справочно: рост нигде не считается, он подсказывает игроку, какой размер '
    + 'выбрать.',
  /** Нижняя граница роста — подпись как в мастерской сайта */
  heightFrom: 'Высота от',
  /** Верхняя граница роста — подпись как в мастерской сайта */
  heightTo: 'Высота до',
  iconPlaceholder: 'tabler:user',
  visionTitle: 'Обычное зрение (фт.)',
  visionHint:
    'Как далеко токен видит в дневном режиме. 0 — без ограничений, как у '
    + 'большинства видов. Тёмное зрение задаётся у особенности.',
...
export const TOKEN_SETTINGS_LABELS = {
  ...
  facingEnabled: 'Вращение',
  facingEnabledHint:
    'Стрелка направления у фишки: за неё токен поворачивают на сцене, а при '
    + 'движении он сам разворачивается по ходу. Без неё ни стрелки, ни поворота.',
  facingForcedHint:
    'Включено всегда: угол обзора меньше 360°, и сектору нужно направление.',
...
export const FEAT_GRANTS_LABELS = {
  /** Заголовок раздела даров на вкладке владений */
  grantsTitle: 'Дары черты',
  /** Подсказка раздела даров: ⓘ в шапке, одной строкой с заголовком */
  grantsHint:
    'Одна строка — одно, что черта даёт. Например: «Крепыш» даёт владение '
    + 'спасбросками Телосложения — это строка «Спасбросок → выдать всё → '
    + 'Телосложение». А «Умелый» даёт выбрать три штуки из навыков и '
    + 'инструментов — это строка «Навык + Инструмент → дать выбрать → 3». '
  ...
  kindHint:
    ...
    + 'Оружие, приёмы, черту и «вариант» смешивать нельзя — их значения берутся '
    + 'из данных мира и компендиума, и в общей куче их не различить, поэтому '
    + 'такой вид остаётся в строке один.',
  /**
   * Чем «Оружие с приёмом» отличается от «Оружейного приёма». Два разных дара, и
   * путать их дорого: первый называет оружие, второй — сам приём.
   */
  kindMasteryHint:
    '«Оружие с приёмом» — выбирают ВИДЫ ОРУЖИЯ, и персонаж получает приём '
    + 'каждого из них: так устроен «Оружейный приём» воина. «Оружейный приём» — '
    + 'выбирают САМ ПРИЁМ, безотносительно оружия: так устроен «Тактический '
    + 'мастер» 9 уровня, подменяющий приём на Толкание, Изнурение или Замедление.',
  ...
  choiceRequiredLevelHint:
    'Уровень персонажа, с которого спрашивают этот выбор. Ноль — сразу.',
  /** Запасной ряд строки дара: у выбора умения он свой */
  choiceScalingEmpty:
  ...
  choiceShortNamePlaceholder: 'Приёмы',

  /** Строка дара вида «Черта» */
  featCategories: 'Категории черт',
  featCategoriesHint:
    'Из каких категорий выбирают: «Боевой стиль» у воина, «Эпическая черта» у '
    + 'умения 19 уровня. Пусто — предложат любую черту, кроме тех, что лист '
    + 'исключает сам (черты происхождения, эпические и боевые стили). '
    + 'Перечень черт ниже сужает пул сильнее категорий.',
  featValues: 'Какие черты выдаются',
  featPool: 'Из каких черт выбирают',
  featPoolHint:
    'Пусто — годится любая черта названных категорий. Перечислите, чтобы задать '
    + 'закрытый список: так «выбери боевой стиль» превращается в «выбери один '
    + 'из этих трёх».',
  featEmpty: 'Черты не выбраны.',

  /** Подписи панели фильтра в окне выбора значений — по основному виду строки */
  filterWeaponCategory: 'Категория',
  filterToolCategory: 'Категория',
  filterMastery: 'Приём',
  filterAbility: 'Характеристика',
  ...
  modifiersHint:
    'То, что запись меняет на листе навсегда. Например: «Крепкий» добавляет 2 '
    + 'хита за каждый уровень, «Бдительный» — бонус мастерства к инициативе, '
    + '«Ловкач» — 10 футов к скорости. Если прибавка работает не всегда («+1 к '
    + 'КД, пока в доспехе»), её место не здесь, а на вкладке «Эффекты»: там у '
  ...
  countersHint:
    'Запас, который тратится и восстанавливается на отдыхе. '
  ...
  counterStartLevel: 'Уровень начала',
  /** Приставка строки ступени: дальше идёт номер уровня */
  counterStepLevel: 'Ур.',
  /** Стрелка между уровнем ступени и её значением */
  counterStepArrow: '→',
  counterStepAdd: 'Добавить ступень',
  counterStepRemove: 'Удалить ступень',
  counterStepsHint:
    'Ступени задают максимум прямо по уровням — тем рядом, который формулой не '
  ...
  counterShowInTableHint:
    ...
    + 'берётся из ступеней или формулы самого ресурса — второй раз его не '
    + 'набирают.',
...
/**
 * Метки вариантов стартового снаряжения класса по порядку. Кириллица — ими
 * подписаны варианты в выгрузке сайта («Вариант А»), и латинская метка рядом с
 * ними читалась бы чужой.
 */
export const CLASS_OPTION_KEYS = 'АБВГДЕЖЗИК';
...
export const STARTING_EQUIPMENT_EDITOR_LABELS = {
  hint:
    'Вариант — это набор предметов и золото к ним. Предметы выбираются из '
    + 'компендиума: по ним мастер кладёт снаряжение в инвентарь.',
  empty: 'Вариантов нет.',
  optionTitle: 'Вариант',
  goldAlternative: 'Альтернатива золотом (зм, 0 = нет)',
  coins: 'Золота (0 = нет)',
  itemsEmpty: 'Предметов нет.',
  itemName: 'Название предмета',
  itemQuantity: 'Кол-во',
  itemNote: 'Уточнение',
  /** Кнопка выбора предметов из компендиума */
  itemPick: 'Добавить предметы',
  /** Кнопка строки, вписанной руками */
  itemAdd: 'Вписать своё',
  itemRemove: 'Убрать предмет',
  optionAdd: 'Добавить вариант',
  optionRemove: 'Удалить вариант',
  pickerTitle: 'Выбор предметов',
  /** Подпись фильтра окна выбора: снаряжение / оружие / инструменты */
  filterItemType: 'Раздел',
  /** Бейдж строки: предмета в компендиумах мира не нашлось */
  missing: 'Своё',
  missingHint:
    'Такого предмета в компендиумах мира нет. В инвентарь он ляжет простым '
    + 'предметом по названию — без веса, стоимости и боевых полей.',
} as const;
...
export const SPELL_LIST_LABELS = {
  ...
  hint:
    'Заклинания, которые запись добавляет в список класса персонажа, — таблица '
    + '«Заклинания метки». Это НЕ выдача и НЕ выбор. Выданное заклинание персонаж знает и накладывает '
    + 'сразу; заклинание отсюда он лишь может выучить или подготовить, как любое '
    + 'из списка своего класса, — мастер показывает такие заклинания в '
    + 'компендиуме сверх фильтра по классу. Если свалить их в одну кучу, «Метка '
    + 'исцеления» выдаст всю таблицу готовыми заклинаниями. Каждая ступень '
    ...
    + 'Круг указывать не надо: он берётся из самой записи.',
...
/**
 * Подписи блока выдачи заклинаний группами ({@link GrantedSpellGroupsEditor}).
 *
 * Тот же блок и те же формулировки, что в мастерской сайта: автор заводит записи
 * и там, и там, и разные подписи одного и того же он читал бы как разные вещи.
 */
export const GRANTED_SPELL_GROUPS_LABELS = {
  hint:
    'Заклинания, которые запись даёт знать сразу. Группами: у каждой свой уровень открытия и свой источник. «Перечисленные» '
    + '— вот эти заклинания и никакие другие. «Весь список класса» — всё, что '
    + 'доступно классу: список собирается при выдаче, поэтому заклинание, '
    + 'заведённое в компендиум позже, достанется новым персонажам само, без '
    + 'правки записи. Группы не заменяют друг друга: каждая открывается на своём '
    + 'уровне и складывается с предыдущими — заклинания домена приходят на 3, 5 '
    + 'и 7 уровнях тремя группами.',
  add: 'Добавить группу',
  remove: 'Удалить группу',
  source: 'Что выдаётся',
  sourceHint:
    'Перечисленные — вот эти заклинания и никакие другие. Весь список класса — '
    + 'всё, что доступно классу в компендиуме: список собирается при выдаче, а '
    + 'не перечисляется заранее.',
  classes: 'Списки классов',
  classesHint:
    'Чьи списки заклинаний выдаются. Несколько классов складываются: это не '
    + 'выбор игрока, а объединение списков. В набор попадают и классы '
    + 'компендиума, включая хоумбрю: сверка идёт по ключу класса, которым '
    + 'помечено заклинание.',
  classesPlaceholder: 'Выберите класс...',
  level: 'Круг',
  levelHint:
  ...
  levelFromRecord: 'из записи',
  requiredLevel: 'С уровня',
  requiredLevelHint:
    'Уровень, с которого приходит ЭТА группа. Пусто — сразу при взятии записи. '
    + 'Не путать с кругом заклинания: круг берётся из самой записи заклинания.',
  requiredLevelPlaceholder: 'сразу',
  ability: 'Характеристика',
  abilityHint:
  ...
  abilityPlaceholder: 'От класса',
  alwaysPrepared: 'Подготавливать не нужно',
  spellPacks: 'Из компендиумов',
  spellPacksHint:
    'Из каких паков брать заклинания. Пусто — из всех доступных, включая '
    + 'заклинания самого мира. Нужно, когда рядом лежат две редакции одного '
    + 'компендиума и список собрался бы из обеих.',
  spellPacksPlaceholder: 'Из всех...',
} as const;

/**
 * Подписи блока «Выбор заклинаний» — того, что игрок берёт сам при взятии
 * записи. Блок общий на все формы, которые дают заклинания: черта, класс,
 * подкласс, вид и предыстория спрашивают выбор одинаково.
 */
export const SPELL_CHOICE_LABELS = {
  /** Заголовок раздела выборов на вкладке заклинаний */
  title: 'Выбор заклинаний',
  hint:
    'Заклинания, которые игрок выбирает сам при взятии записи. Так устроен «Посвящённый в магию»: список класса — жреца, друида или '
    ...
    + 'заводит сама, когда классов больше одного. Порция может вместо круга и '
    + 'списков классов перечислять конкретные заклинания — тогда игрок выбирает '
    + 'только из них. Выбранное ложится в книгу так же, как выданное чертой без '
    + 'выбора.',
  ...
  classesHint:
    ...
    + 'перечисленных. Пусто — подойдёт заклинание любого класса. Поле относится '
    + 'только к порциям с поиском по кругу: у порций с перечисленными '
    + 'заклинаниями класс берётся из самой записи.',
  ...
  count: 'Сколько',
  /** Откуда порция берёт пул: поиском по кругу и спискам либо из перечисленных */
  source: 'Откуда выбирают',
  sourceHint:
    '«По кругу и спискам классов» — пул собирается из компендиума: все заклинания '
    + 'названного круга из списков выбранных классов. «Из перечисленных '
    + 'заклинаний» — игрок выбирает ровно из тех записей, что добавлены ниже; '
    + 'круг и класс у них свои.',
  sourceFilter: 'По кругу и спискам классов',
  sourceList: 'Из перечисленных заклинаний',
  levelFromRecord: 'по записям',
  listedSpells: 'Заклинания, из которых выбирают',
  requiredLevel: 'С уровня',
  requiredLevelPlaceholder: 'сразу',
  requiredLevelHint:
    'Уровень персонажа, с которого порцию спрашивают. Пусто — сразу при взятии. '
    + 'Нужен записи, которая спрашивает порции на разных уровнях: «Таинственный '
    + 'арканум» даёт заклинание 6 круга на 11 уровне, 7 круга — на 13.',
  ...
  grantedSpellsAlwaysPrepared: 'Подготавливать не нужно',
...
export const CLASS_EQUIPMENT_STEP_LABELS = {
  ...
  /** Метка отказного варианта — он идёт третьим и есть у любого класса */
  noneTitle: 'Без снаряжения',
  noneDescription:
    'Ничего не добавлять — инвентарь останется как есть. Снаряжение '
    + 'выдадите вручную из справочника.',
} as const;

/**
 * Значение выбранного варианта снаряжения, когда игрок отказался от выдачи.
 * От «ничего не выбрано» (`null`) отличается только тем, что отказ сказан
 * вслух: карточка подсвечена, и мастер не ждёт от игрока выбора.
 */
export const CLASS_EQUIPMENT_NONE_INDEX = -1;
...
/**
 * С какого размера пула в окне выбора появляется поиск. Короткий список
 * прокручивается глазами быстрее, чем набирается запрос, а два десятка
 * заговоров волшебника глазами уже не берут.
 */
export const CHOICE_PICKER_SEARCH_LIMIT = 8;

/**
 * Подписи общего выбора в мастерах персонажа: строка со счётчиком и кнопкой
 * «Выбрать» и само окно выбора.
 *
 * Один набор на все выборы листа — заклинания, навыки, инструменты, варианты
 * умений, черту, происхождение. Раньше каждый вид выбора говорил о себе
 * по-своему («Выбрано 1 из 1» против «выбрано 1 из 1»), а игрок за одно
 * повышение уровня проходит их подряд и разнобой читает как разные механики.
 */
export const CHOICE_PICKER_LABELS = {
  /** Кнопка, открывающая окно выбора */
  open: 'Выбрать',
  /** Приставка счётчика выбранного: между ней и пределом идут числа */
  chosenPrefix: 'Выбрано ',
  chosenMiddle: ' из ',
  /** Ничего ещё не отмечено — вместо плашек выбранного */
  nothingChosen: 'ничего не выбрано',
  /** Поиск по списку окна: он появляется только у длинных пулов */
  searchPlaceholder: 'Начните вводить название...',
  /** Поиск ничего не нашёл */
  nothingFound: 'Ничего не найдено',
  /** Кнопка описания варианта и запасной заголовок его окна */
  detailOpen: 'Показать описание',
  detailTitle: 'Описание варианта',
  /** Пометка варианта, который берут повторно */
  repeatableBadge: 'можно повторно',
  /**
   * Требования варианта живой фразой: лист их не проверяет, поэтому строка
   * показывается игроку как есть — решает он сам.
   */
  prerequisitePrefix: 'Требования: ',
  /** Снять все отметки — когда выбор набирают не с нуля */
  clear: 'Снять всё',
} as const;
...
export const FEAT_CHOICES_LABELS = {
  ...
  /**
   * Пул пуст по другой причине: вариантов у выбора нет вовсе. Отдельная подпись
   * нужна, потому что про владения тут говорить нечем — так выглядит незагруженный
   * справочник или запись, забывшая перечислить, из чего выбирают.
   */
  emptyPoolNoOptions: 'Выбирать не из чего: у этого выбора нет вариантов.',
  /**
  ...
  rechooseHint:
    ...
    + 'или поменяйте выбор.',
  /** Заголовок того же окна, когда повод другой: на новом уровне открылся выбор */
  leveledTitle: 'Новые выборы черт',
  leveledHint:
    'На новом уровне у черт открылись новые выборы. Ответьте на них сейчас или '
    + 'закройте окно, чтобы вернуться позже.',
} as const;
...
export const ACTOR_SHEET_LABELS = {
  ...
  classRemovedSuffix: ' и все связанные умения и эффекты удалены',
} as const;
...
/**
 * Подписи круга заклинания. Общие: круг подписан одинаково в списках мастера,
 * в окне выбора и в плашке выбранного — игрок видит их подряд, и два написания
 * одного круга читались бы как разные вещи. Формат см. `spellCircleLabel`.
 */
export const SPELL_CIRCLE_LABELS = {
  /** Нулевой круг называют словом, а не числом */
  cantrip: 'заговор',
  /** Приписка круга: «3» + это = «3 кр.» */
  levelSuffix: ' кр.',
} as const;

/**
 * Подписи шага заклинательства в мастере класса: характеристика, заговоры,
 * подготовленные и ячейки. Отдельно от подписей формы класса: там значения
 * задают, здесь показывают уже посчитанные для этого уровня.
 */
export const WIZARD_SPELLCASTING_LABELS = {
  /** Подпись плитки заклинательной характеристики */
  ability: 'Характеристика',
  abilityHint:
    'Заклинательная характеристика класса: от неё считаются сложность '
    + 'спасброска от заклинаний и бонус атаки заклинанием.',
  cantrips: 'Заговоры',
  cantripsHint:
    'Сколько заговоров персонаж знает на этом уровне. Заговоры накладываются '
    + 'сколько угодно раз и ячеек не тратят.',
  prepared: 'Подгот.',
  preparedHint:
    'Сколько заклинаний персонаж держит подготовленными одновременно. Набор '
    + 'пересматривается на продолжительном отдыхе.',
  /** Подпись плитки ячеек: «1» + это = «1 круг» */
  slotLevelSuffix: ' круг',
  /** Приписка к подписи ячеек, когда они пактовые */
  pactSuffix: ' · пакт',
  /**
   * Пояснение плитки ячеек. Между приставкой и хвостом идёт круг: «Ячейки
   * заклинаний 1 круга — их тратят…».
   */
  slotHintPrefix: 'Ячейки заклинаний ',
  slotHintSuffix:
    ' круга — их тратят, когда накладывают заклинание этого круга. '
    + 'Восстанавливаются на продолжительном отдыхе.',
  pactHint:
    'Пактовая магия колдуна: все ячейки одного круга, и восстанавливаются они '
    + 'уже на коротком отдыхе.',
  /** Строки «сколько взять»: перед двоеточием — что берут, после — число */
  chooseCantrips: 'Новых заговоров',
  /** Строка общего лимита: между приставкой и хвостом идут доступные круги */
  chooseSpellsPrefix: 'Новых заклинаний ',
  /** Строка покругового лимита: «Заклинаний 3 круга» */
  chooseByLevelPrefix: 'Заклинаний ',
  /** Общий хвост обеих строк — круг, о котором речь */
  circleSuffix: ' круга',
  /**
   * Откуда берутся заклинания и зачем их выбирают здесь. Между приставкой и
   * хвостом идёт название класса.
   */
  sourceHintPrefix: 'Выбирают из списка заклинаний класса «',
  sourceHintSuffix:
    '»: на этом уровне персонаж пополняет им свою книгу. Кнопка откроет '
    + 'компендиум с уже выставленными фильтрами по классу и доступным кругам.',
  /** Приписка к пояснению, когда список расширен записями листа */
  sourceHintExpanded:
    ' Сверх списка класса доступно то, что открыли умения, черты и вид, — в '
    + 'компендиуме такие заклинания идут первыми.',
  viewSpells: 'Посмотреть заклинания',
  manualHint:
    'Выбор конкретных заклинаний доступен в разделе заклинаний персонажа.',
  title: 'Заклинания',
  /** Заголовок блока с выданным умениями; число идёт бейджем в шапке */
  grantedTitle: 'Заклинания от умений',
  grantedHint:
    'Эти заклинания уже выданы умениями и не тратят лимит выбора. Нажмите на '
    + 'заклинание, чтобы прочитать его карточку.',
  /** Приставка записи-источника в заголовке группы списка */
  featurePrefix: GRANTED_SPELL_FEATURE_PREFIX,
  /** Заголовок блока сводки уровня — плиток характеристики и ячеек */
  summaryTitle: 'Заклинательство на этом уровне',
  /** Заголовок блока, в котором выбирают новые заклинания */
  chooseTitle: 'Выбор заклинаний',
  selectedTitle: 'Выбранные заклинания',
  selectedHint:
    'Заклинания, которые вы взяли на этом уровне. Нажмите на заклинание, чтобы '
    + 'прочитать его карточку, или на крестик, чтобы снять выбор.',
  /** Заголовок блока с заклинаниями сверх списка класса */
  expandedTitle: 'Доступны сверх списка класса',
  expandedHint:
    'Расширение списка от умений, черт и вида: эти заклинания можно выбрать в '
    + 'компендиуме наравне с классовыми.',
  /** Подпись кнопки, снимающей выбранное заклинание */
  removeSpell: 'Убрать заклинание',
  /** Подпись кнопки-плашки заклинания */
  openSpell: 'Открыть карточку заклинания',
} as const;

/** Подписи списка особенностей формы вида (сворачиваемые строки). */
export const SPECIES_FEATURES_EDITOR_LABELS = {
  add: 'Добавить особенность',
  fallbackName: 'Новая особенность',
...
export const SPECIES_FEATURE_LABELS = {
  ...
  spellChoices: 'Заклинания на выбор (игрок выбирает сам)',
  spellList: 'Расширение списка заклинаний класса',
} as const;
...
export const PROFICIENCY_MODAL_LABELS = {
  ...
  /** Приёмы, которыми персонаж владеет сам по себе, а не через оружие */
  masteryProperties: 'Приёмы без привязки к оружию',
...
export const GRANTED_SPELLS_LABELS = {
  ...
  hint:
    'Заклинание выбирается из компендиума и выдаётся автоматически; если оно '
    + 'есть в нескольких компендиумах — можно выбрать, из какого брать. '
    + 'Имя-ссылка открывает его.',
} as const;
...
export const LEVEL_UP_LABELS = {
  ...
  removeClassHint: 'Все умения класса будут утеряны',
...
/**
 * Сколько полос рисует скелетон на ленте шагов мастера класса. Точного числа
 * шагов до загрузки класса не знает никто: оно зависит от того, первый это
 * класс, мультикласс или повышение уровня.
 */
export const WIZARD_SKELETON_STEPS = [1, 2, 3, 4, 5] as const;
...
export const WIZARD_GRANTS_LABELS = {
  ...
  choice: 'Выбор',
  /** Приставка строки выбора — дальше идут число даров и двоеточие */
  choosePrefix: 'Выберите',
} as const;

/** Подписи редактора умений класса */
export const CLASS_FEATURES_EDITOR_LABELS = {
  collapse: 'Свернуть',
  expand: 'Развернуть',
  /** Подпись узла, когда умению ещё не дали названия */
  fallbackName: 'Умение',
  remove: 'Удалить умение',
  add: 'Добавить умение',
  /**
   * Бейджи свёрнутой строки: что у умения заполнено, не разворачивая его.
   * После приставки идёт число.
   */
  mechanicsBadge: 'механика: ',
  scalingBadge: 'рост: ',
  choicesBadge: 'вариантов: ',
  /** Бейдж выбираемого списка: «выбор: 1–3 из 8» */
  choiceBadge: 'выбор: ',
  choiceBadgeOf: 'из',
  informationalBadge: 'информирует',
  /** Вопрос окна подтверждения — между приставкой и концовкой идёт название */
  removeConfirmPrefix: 'Удалить умение «',
  removeConfirmSuffix:
    '»? Вместе с ним пропадут его дары, ресурсы, эффекты и заклинания.',
} as const;

/**
 * Стартовое название нового умения класса. Значение записи, а не подпись:
 * оно уходит в сохранённый класс.
 */
export const CLASS_FEATURE_DEFAULT_NAME = 'Новое умение';
...
export const WIZARD_ASI_LABELS = {
  ...
  featHint:
    'Черта берётся вместо повышения характеристик: по правилам 2024 года само повышение — тоже черта.',
  featEmpty:
    'Подходящих черт в компендиуме нет — проверьте, загружен ли пак черт.',
  /** Счётчик нераспределённых очков — дальше идёт само число */
  pointsRemainingPrefix: 'Осталось: ',
} as const;

/**
 * Категории черт, которые не предлагаются выбору черты без ограничения
 * категорий: черты происхождения даёт предыстория, эпические — умение
 * 19 уровня, боевые стили — своё умение. Подписями записи компендиума
 * (`Feature.category`) — так их отдаёт выгрузка и так сверяет пикер.
 */
export const CLASS_FEAT_DEFAULT_EXCLUDED_CATEGORIES: ReadonlyArray<string> = [
  'Черта происхождения',
  'Эпическая черта',
  'Боевой стиль',
];
...
/**
 * То же для выбора черты в дарах ВИДА. Черта происхождения здесь не исключена:
 * дар вида — как раз тот случай, когда её и выбирают («Универсальность»
 * человека предлагает черту происхождения на выбор). Записи, назвавшие
 * категории сами, это правило не касается.
 */
export const SPECIES_FEAT_DEFAULT_EXCLUDED_CATEGORIES: ReadonlyArray<string> = [
  'Эпическая черта',
  'Боевой стиль',
];
...
export const CLASS_WIZARD_LABELS = {
  ...
  featuresEmpty: 'На этом уровне нет новых умений.',
  /** Заголовок выбора подкласса, когда своего названия группе не задали */
  subclassFallback: 'Выбор подкласса',
  /** Уведомление о переносе на лист записи-подкласса без её родителя */
  subclassParentMissingTitle: 'Это подкласс',
  subclassParentMissingText:
    'Родительский класс этого подкласса не найден среди записей — подключите '
    + 'пак с ним и начните с родительского класса.',
  /** Пояснение к пикерам черт умений уровня: боевой стиль и подобные */
  featPicksTitle: 'Выбор черты умением',
  /** Свёрнутая строка умения уровня: описание читают, развернув её */
  featureExpand: 'Развернуть умение',
  featureCollapse: 'Свернуть умение',
  /** Пометка строки умения, у которого игрока ещё спрашивают вариант */
  choiceNeededBadge: 'нужен выбор',
  choiceDoneBadge: 'выбор сделан',
  /** Взятые на прошлых уровнях варианты — их второй раз не берут */
  choiceTakenTitle: 'Уже взято:',
  /**
   * Заголовок доборов: умение получено раньше, а варианты к нему добираются на
   * этом уровне (воззвания колдуна).
   */
  choicePicksTitle: 'Выборы прошлых умений',
...
export const COMPENDIUM_LABELS = {
  ...
  dragHint:
    'Перетащите на стол, чтобы поставить токен, или на правую панель — чтобы просто скопировать в мир',
  /**
   * Подсказка перетаскивания записи, которая на столе ничем не станет
   * (заклинание, класс, вид, предыстория, черта, предмет): её принимает только
   * правая панель.
   */
  dragHintEntry: 'Перетащите на правую панель, чтобы скопировать в мир',
  known: 'Изучено',
  nothingFound: 'Ничего не найдено',
  noData: 'Нет данных',
...
export const CLASS_FEATURE_LABELS = {
  ...
  scalingHint:
    'Уровни, на которых умение повторяется или усиливается: «Дополнительная '
    + 'атака» на 11-м, «Улучшение характеристик» на 8, 12 и 16. Каждая ступень '
    + 'уходит в запись класса отдельным умением своего уровня — так же, как её '
    + 'разворачивает выгрузка сайта.',
  scalingAdd: 'Добавить уровень',
  scalingRemove: 'Убрать уровень',
  scalingLevel: 'Уровень',
  scalingName: 'Название',
  scalingNamePlaceholder: 'Пусто — как у умения',
  scalingDescription: 'Описание',
  scalingDescriptionPlaceholder: 'Что меняется на этом уровне',
  ...
  mechanicsHint:
    'Что умение делает на листе персонажа: выдаёт владения и заклинания, даёт '
    + 'выбрать навык или черту, заводит ресурс со счётчиком, меняет числа '
    + 'листа эффектом.',
} as const;

/**
 * Подписи настройки выбора из вариантов умения: выбираемый список против
 * справочного, сколько вариантов берут и как это число растёт по уровням.
 */
export const CLASS_FEATURE_CHOICE_CONFIG_LABELS = {
  ...
  labelPlaceholder: 'Пусто — название умения',
  scalingHint:
    'Ступень называет, сколько вариантов выбрано ВСЕГО к этому уровню, а не '
    + 'сколько добавилось: у колдуна одно воззвание с первого уровня и три со '
    + 'второго — значит, на втором он выбирает два новых.',
  scalingEmpty:
    'Количество не растёт — выбирают столько же, сколько на уровне умения.',
} as const;

/**
 * Подписи строки варианта умения — тот же набор полей, что у варианта в
 * мастерской сайта: без них выгруженный вариант терял бы при правке всё, кроме
 * названия и описания.
 */
export const CLASS_FEATURE_CHOICE_LABELS = {
  name: 'Название',
  namePlaceholder: 'Название варианта',
  nameEn: 'Английское название',
  nameEnPlaceholder: 'English name',
  level: 'С уровня',
  levelPlaceholder: 'сразу',
  levelHint:
    'Уровень класса, с которого вариант вообще можно взять. Пусто — сразу '
    + 'вместе с умением. Мастер уровня вариант с уровнем выше не предлагает.',
  repeatable: 'Можно взять повторно',
  repeatableHint:
    'Вариант остаётся в списке, даже если игрок уже его брал. Без галочки '
    + 'взятый вариант из следующего выбора уходит.',
  hideInSubclasses: 'Скрывать в подклассе',
  hideInSubclassesHint:
    'Вариант не показывается на странице подкласса. На листе персонажа он '
    + 'спрашивается там же, где заведён.',
  ...
  /** Бейдж уровня доступа в свёрнутой строке: «С 5 уровня» */
  levelBadgePrefix: 'С ',
  levelBadgeSuffix: ' уровня',
  repeatableBadge: 'повторно',
  hiddenBadge: 'скрыт в подклассе',
  mechanicsTitle: 'Механика и эффекты варианта',
  mechanicsHint:
...
/**
 * Разделитель имени умения и его варианта в записи на листе: «Таинственные
 * воззвания: Договор гримуара».
 *
 * Одной строкой на всех, потому что имя записи и собирают, и разбирают: мастер
 * класса склеивает её при выдаче, а снятие класса по ней же находит заклинания
 ...
 * Покровитель первородный».
 *
 * Тоже на всех: мастер класса склеивает происхождение при выдаче, а снятие
 * класса разбирает его обратно — заклинания подкласса помечены его именем.
 */
export const FEATURE_SOURCE_SEPARATOR = ' — ';

/**
 * Подписи вложенного раздела формы. Разделов в записи несколько подряд, и
 * подпись кнопки свёртки у них общая.
 */
export const EDITOR_NESTED_SECTION_LABELS = {
  expand: 'Развернуть блок',
  collapse: 'Свернуть блок',
} as const;

/**
 * Подписи блока «Механика и эффекты» у самого умения класса.
 *
 * Блок один на двоих с вариантом умения, а называть источник даров в подписях
 * нужно по-разному: «Дары умения» внутри варианта вводили бы в заблуждение —
 * выдаёт их вариант.
 */
export const CLASS_FEATURE_MECHANICS_TITLES = {
  grants: 'Дары умения',
  grantsHint:
    'Что выдаёт само умение: владения, языки, чувства, выборы игрока. Дары '
    + 'всего класса задаются на вкладке «Дары».',
  modifiers: 'Правки листа от умения',
  modifiersHint:
    'То, что умение меняет на листе навсегда: прибавка к скорости, к хитам, к '
    + 'инициативе. Прибавка с условием («пока в доспехе») — не сюда, а в '
    + 'эффекты умения: там у неё есть условие.',
  counters: 'Ресурсы умения',
  countersHint:
    'Запас умения, который тратится и восстанавливается на отдыхе: заряды '
    + 'вдохновения, очки ярости. Ресурс умения приходит вместе с самим умением '
    + 'и просится колонкой в таблицу прогрессии класса.',
  spells: 'Заклинания умения',
  spellsHint: 'Выданные умением заклинания всегда подготовлены',
  spellChoice: 'Заклинания на выбор игрока',
  spellChoiceHint:
    'Умение не выдаёт заклинание, а даёт выбрать его самому: «Договор Гримуара» '
    + 'предлагает три заговора из списка любого класса. Мастер класса спросит '
    + 'выбор на уровне умения.',
  spellList: 'Расширение списка заклинаний умением',
  spellListHint:
    'Заклинания, которые умение лишь позволяет подготовить наравне с '
    + 'классовыми, — не выдаёт. Подготовку и ячейку персонаж тратит сам.',
  effects: 'Эффекты умения',
  effectsHint:
    'Эффекты этого умения. Переносятся на персонажа на уровне, где умение '
    + 'получено.',
} as const;

/** Те же подписи для блока механики варианта умения. */
export const CLASS_OPTION_MECHANICS_TITLES = {
  grants: 'Дары варианта',
  grantsHint:
    'Что выдаёт сам вариант, когда его выбрали: владения, языки, чувства, '
    + 'выборы игрока. Дары самого умения задаются выше, у умения.',
  modifiers: 'Правки листа от варианта',
  modifiersHint:
    'То, что выбранный вариант меняет на листе навсегда. Прибавка с условием '
    + '(«пока в доспехе») — не сюда, а в эффекты варианта: там у неё есть '
    + 'условие.',
  counters: 'Ресурсы варианта',
  countersHint:
    'Запас варианта, который тратится и восстанавливается на отдыхе. В таблицу '
    + 'прогрессии класса он не идёт: вариант могут и не взять, а колонка '
    + 'таблицы стоит у всех.',
  spells: 'Заклинания варианта',
  spellsHint:
    'Заклинания, которые даёт выбранный вариант: воззвание колдуна выдаёт своё '
    + 'заклинание. Они всегда подготовлены.',
  spellChoice: 'Заклинания варианта на выбор игрока',
  spellChoiceHint:
    'Вариант не выдаёт заклинание, а даёт выбрать его самому: «Договор '
    + 'Гримуара» предлагает три заговора из списка любого класса. Спрашивается '
    + 'только когда вариант взят.',
  spellList: 'Расширение списка заклинаний вариантом',
  spellListHint:
    'Заклинания, которые вариант лишь позволяет подготовить наравне с '
    + 'классовыми, — не выдаёт. Подготовку и ячейку персонаж тратит сам.',
  effects: 'Эффекты варианта',
  effectsHint:
    'Эффекты этого варианта. Переносятся на персонажа, только когда вариант '
    + 'выбран.',
} as const;
...
export const CLASS_DETAIL_LABELS = {
  ...
  fallbackTitle: 'Класс',
  /** Пометка умения, которое даёт подкласс */
  subclassFeature: 'Умение подкласса',
  /** Приставка названия подкласса в списке умений */
  subclassPrefix: 'Подкласс ',
  /** Пустой список владений в карточке */
  empty: 'Нет',
  hitDie: 'Кость хитов',
  primaryAbilities: 'Основная характеристика',
  ...
  choicesTitle: 'Варианты',
  /** Сколько вариантов берут из списка — дальше идёт число или диапазон */
  choiceCountPrefix: 'Выбирают: ',
  /** Требования варианта — дальше идёт их текст из записи класса */
  choicePrerequisitePrefix: 'Требования: ',
...
export const CLASS_LEVEL_TABLE_LABELS = {
  ...
  columnSpells: 'Закл.+',
  /** Колонка умений уровня — общая у редактора таблицы и карточки класса */
  columnFeatures: 'Умения',
  hint:
    '«Умения» уровня выводятся автоматически из уровня каждого умения '
    + '(вкладка «Умения»). Пустая ячейка колонки = «—». Бонус мастерства '
...
/** Наименьшее количество выбираемых вариантов: выбор из нуля бессмыслен. */
export const CHOICE_COUNT_MIN = 1;

/**
 * Подписи ступеней роста количества выбора. Ряд задают в двух местах — у
 * строки даров и у настройки выбора из вариантов умения, — и подписи у них
 * общие: считают они одно и то же.
 */
export const CHOICE_SCALING_LABELS = {
  title: 'Рост количества по уровням',
  level: 'С уровня',
  count: 'Всего',
...
/**
 * Только что включённый выбор из вариантов умения: один вариант — так устроено
 * большинство списков, а количество побольше автор поправит сам. То же
 * значение по умолчанию ставит мастерская сайта.
 */
export const CHOICE_CONFIG_DEFAULT_COUNT = 1;
...
export const BACKGROUND_FORM_LABELS = {
  ...
  /** Видимое пояснение секции снаряжения: когда игрок выбирает вариант */
  equipmentHelp:
    'Варианты стартового снаряжения — один из них игрок выбирает в мастере '
    + 'предыстории.',
  grantsHint:
...
export const CLASS_FORM_LABELS = {
  ...
  effectsHint:
    'Активные эффекты (бонусы к характеристикам, КД, флаги и т.п.). '
    + 'Переносятся на персонажа при взятии первого уровня в классе.',
  /**
  ...
  // Подпись та же, что на сайте: одна и та же таблица класса не должна
  // называться в двух местах по-разному
  tabProgression: 'Таблица',
  tabGrants: 'Дары',
  grantsTitle: 'Дары класса',
  grantsHint:
    'То, что даёт взятие класса целиком: владения, выборы, правки листа и '
    + 'ресурсы. Дары конкретного умения задаются у самого умения.',
  spellsHint:
    'Заклинания, которые персонаж знает сразу при взятии класса, — без выбора. '
    + 'Заклинания конкретного умения задаются у самого умения.',
  /**
   * Вкладка умений класса. Не «Особенности»: умением их зовут и мастерская
   * сайта, и выгрузка, и остальные подписи самой формы («Дары умения»).
   */
  tabFeatures: 'Умения',
  ...
  subclassesSectionTitle: 'Подклассы',
  /** Тултип блока подклассов: чем подпись группы отличается от уровня выбора */
  subclassesSectionHint:
    'Подпись группы («Воинский архетип») и уровень, на котором мастер '
    + 'повышения предложит выбрать подкласс. У записи-подкласса блока нет: '
    + 'подкласс подкласса модель не знает.',
  parent: 'Родительский класс',
  parentNone: 'Самостоятельный класс',
  parentPick: 'Выбрать',
  parentPickTitle: 'Выбор родительского класса',
  parentClear: 'Убрать родительский класс',
  /** Подсказка поля «Родительский класс»: что меняется у записи с родителем */
  parentHint:
    'Задан — запись не самостоятельный класс, а его подкласс: лист покажет её '
    + 'среди подклассов родителя, а не в списке классов.',
  abilitiesSectionTitle: 'Характеристики класса',
  primaryAbilities: 'Основная характеристика',
  primaryAbilitiesPlaceholder: 'Характеристики класса...',
  /** Тултип поля основных характеристик: зачем они листу */
  primaryAbilitiesHint:
    'Что класс повышает в первую очередь; по ним же считаются требования '
    + 'мультиклассирования.',
  primaryAbilitiesDelimiter: 'Разделитель',
  ...
  startingProficienciesHint:
    ...
    + 'сокращённый набор из секции «Мультикласс», а спасброски (вкладка '
    + '«Основное») не выдаются вовсе.',
  armorPlaceholder: 'Владение доспехами...',
  weaponsPlaceholder: 'Владение оружием...',
  toolsPlaceholder: 'Владение инструментами...',
  /** Приписка к владению: то, что категориями не выражается */
  proficiencyCustom: 'Дополнительно',
  armorCustomPlaceholder: 'Например: только щиты',
  weaponsCustomPlaceholder: 'Например: длинные мечи',
  ...
  subclassesHint:
    ...
    + 'подкласса» (вкладка «Основное»). Умения и счётчики подкласса '
  ...
  equipmentHint:
    'Варианты стартового снаряжения — один из них игрок выбирает при взятии '
    + 'класса на 1 уровне.',
