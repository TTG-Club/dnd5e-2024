...
/**
 * Подпись итогового числа листа рядом с расчётом окна настройки. Окно считает
 * по своим полям, а лист — по всему сразу (снаряжение, свои бонусы, эффекты),
 * и разойтись эти числа могут без всякой поломки: подпись поэтому нейтральная,
 * без «виноваты эффекты».
 */
export const SHEET_TOTAL_LABEL = 'Итог на листе';

/**
 * Пометка строки эффекта, которая действует не всегда. Часть условий лист
 * считает сам (доспех носителя, тип существа), часть откладывает до броска —
 * решает это движок, и окно настройки лишь помечает такую строку.
 */
export const CONDITIONAL_CHANGE_MARK = 'по условию';
...
export const ARMOR_CLASS_SETTINGS_LABELS = {
  ...
  /**
   * Итог листа рядом с расчётом окна: окно не знает ни про надетую броню, ни
   * про щит, ни про эффекты, а плитка листа считает их все.
   */
  sheetTotal: SHEET_TOTAL_LABEL,
  /** Шапка разбора итога */
  sheetTotalHint: 'Считается по снаряжению, своим бонусам и эффектам',
...
export const MOVEMENT_SETTINGS_LABELS = {
  ...
  units: 'Единицы',
  hover: 'Парение',
  /** Шапка разбора итога: под ней перечислены эффекты, тронувшие вид */
  sheetTotalHint: `${SHEET_TOTAL_LABEL} — со своими бонусами и эффектами`,
  /** Строка эффекта, гасящего передвижение целиком */
  zeroBySource: 'скорость обнулена',
  conditionalMark: CONDITIONAL_CHANGE_MARK,
  bonusesHint:
...
export const SAVING_THROW_ROLL_LABELS = {
  ...
  failureSuffix: ' ✗ Провал',
  /**
   * Хвост сообщения в чат, когда окно спасброска закрыли, не бросив: действие
...
export const NO_AMMUNITION_TYPE = 'none';
...
export const FEAT_GRANTS_LABELS = {
  ...
  modifiersHint:
    'Короткий список вечных прибавок: хиты, скорости, класс доспеха, '
    + 'инициатива, чувства, защита от урона, иммунитет к состоянию. Например: '
    + '«Крепкий» добавляет 2 хита за каждый уровень, «Бдительный» — бонус '
    + 'мастерства к инициативе, «Ловкач» — 10 футов к скорости. Чего в списке '
    + 'нет — навыки, спасброски, характеристики, атака, урон — задаётся на '
    + 'вкладке «Эффекты». Там же задаётся и всё отсюда: здесь короче, а эффект '
    + 'нужен, когда прибавка работает не всегда («+1 к КД, пока в доспехе») — '
    + 'у эффекта есть условие.',
 ...
 * видит только итог, поэтому строка называет причину: «нет целей» без
 * объяснения читается как поломка заклинания.
 */
export const SPELL_NO_TARGETS_LABELS = {
  /** Область поставлена, но внутри никого */
  emptyArea: 'в области никого нет',
  /** Ни области, ни выбранной цели */
...
export const PROJECTILE_PROMPT_LABELS = {
  ...
  /**
   * Единственная инструкция окна: снаряды раздаются кликами по фишкам на карте,
   * и без неё окно выглядит как обычное подтверждение каста — игрок жмёт
...
export const CLASS_WIZARD_LABELS = {
  ...
  /**
   * Пометка строки, в которой ещё не ответили: за приставкой идёт число МЕСТ,
   * где игрока спрашивают, — «нужно сделать выбор: 2». Не число значений:
   * «три заговора» — это одно место, куда игрок зайдёт один раз, и тройка в
   * пометке читалась бы как три отдельных дела.
   */
  pendingBadgePrefix: 'нужно сделать выбор: ',
  choiceDoneBadge: 'выбор сделан',
  /** Пометка строки, за которой стоит сама запись класса, а не её умение */
  ownGrantsBadge: 'дары записи',
  ...
  openGrantedSpell: 'Открыть карточку заклинания',
...
export const COUNTER_RESOURCE_LABELS = {
  ...
  /**
   * Уровень в классе-владельце ресурса. У ресурса без класса (свой ресурс
   * игрока, ресурс черты) считать не от чего, и он читается как общий уровень —
   * подпись говорит об этом прямо, чтобы выбор не обещал лишнего.
   */
  sourceClassLevel: 'Уровень в классе (у своего ресурса — общий)',
...
export const HIT_POINTS_LABELS = {
  ...
  /**
   * Итог листа рядом с полем «Всего»: в окне правится СВОЙ запас листа, а
   * плитка показывает его же с прибавкой эффектов.
   */
  sheetTotal: SHEET_TOTAL_LABEL,
  /** Шапка разбора итога */
...
export const CLASS_FEATURE_MECHANICS_TITLES = {
  ...
  modifiersHint:
    'Короткий список вечных прибавок от умения: хиты, скорости, класс доспеха, '
    + 'инициатива, чувства, защита от урона, иммунитет к состоянию. Чего в '
    + 'списке нет — навыки, спасброски, характеристики, атака, урон — задаётся '
    + 'эффектами умения. Там же задаётся и всё отсюда: здесь короче, а эффект '
    + 'нужен, когда прибавка работает не всегда («пока в доспехе») — у эффекта '
    + 'есть условие.',
...
export const CLASS_OPTION_MECHANICS_TITLES = {
  ...
  modifiersHint:
    'Короткий список вечных прибавок от выбранного варианта: хиты, скорости, '
    + 'класс доспеха, инициатива, чувства, защита от урона, иммунитет к '
    + 'состоянию. Чего в списке нет — навыки, спасброски, характеристики, '
    + 'атака, урон — задаётся эффектами варианта. Там же задаётся и всё '
    + 'отсюда: здесь короче, а эффект нужен, когда прибавка работает не всегда '
    + '(«пока в доспехе») — у эффекта есть условие.',
...
export const WEAPON_FORM_LABELS = {
  ...
  saveHint:
    ...
    + 'атаки этого оружия.',
  /** Блок «Расход» у оружия, которое само стреляет боеприпасами */
  firesAmmunitionHint:
    'Оружие стреляет боеприпасами — их тип выбран в «Свойствах». Боеприпасом '
    + 'для другого оружия оно быть не может.',
