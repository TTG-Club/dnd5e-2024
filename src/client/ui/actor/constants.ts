...
export const FEAT_GRANTS_LABELS = {
  ...
  /**
   * Тултип секции повышения характеристик: фикс применяется сам, выбор —
   * вручную.
   */
  asiSectionHint:
    'Фиксированные прибавки лист применяет сам при взятии черты. Прибавку '
    + '«на выбор» он не применяет — она показывается игроку подсказкой, '
    + 'характеристику поднимают вручную.',
  ...
  /**
   * Тултип секции защит и чувств: что применяет движок, а что остаётся
   * справкой.
   */
  defensesSectionHint:
    'Защиты от урона лист учитывает при расчёте получаемого урона '
    + 'автоматически. Из чувств на зрение токена влияет только тёмное '
    + 'зрение — остальные чувства и телепатия показываются на листе справкой.',
  damageDefenses: 'Защиты от типов урона',
  /** Заголовок списка дистанций: чувства и телепатия идут одними строками */
  sensesTitle: 'Чувства и телепатия (фт.)',
  sensesHint: 'Ноль — чувство не даётся.',
  /** Тёмное зрение в списке дистанций: единицы и ноль объяснены заголовком */
  darkvisionShort: 'Тёмное зрение',
  telepathyRange: 'Телепатия',
  modifiersTitle: 'Модификаторы листа',
  modifiersHint:
    'Постоянные прибавки, которые черта даёт без условий и активации. '
    + 'Условные («+1 КД в доспехе») заводятся активным эффектом на вкладке «Эффекты».',
  speedTitle: 'Передвижение (фт.)',
  speedWalkBonus: 'Ходьба, прибавка',
  speedWalkHint: 'к базовой скорости',
  speedFly: 'Полёт',
  speedClimb: 'Лазание',
  speedSwim: 'Плавание',
  speedFlyEqualsWalk: 'равен ходьбе',
  speedClimbEqualsWalk: 'равно ходьбе',
  speedSwimEqualsWalk: 'равно ходьбе',
  hitPointsTitle: 'Максимум хитов, прибавка',
  hitPointsFlat: 'Постоянная',
  hitPointsPerAcquisitionLevel: 'За уровень при взятии',
  hitPointsPerLevelAfterAcquisition: 'За каждый следующий уровень',
  hitPointsHint:
    'Итог: постоянная + «за уровень при взятии» × уровень взятия черты + '
    + '«за каждый следующий уровень» × уровни, полученные после.',
  defenceTitle: 'Защита и инициатива',
  armorClassBonus: 'Класс доспеха, прибавка',
  initiativeBonus: 'Инициатива, прибавка',
  initiativeProficiencyBonus: '+ бонус мастерства',
  /** Кнопка обнуления блока модификаторов — видна, когда что-то задано */
  modifiersReset: 'Сбросить модификаторы',
  /**
   * Заголовок секции требований — тем же словом их зовут карточка черты и
   * предупреждение листа.
   */
  prerequisitesTitle: 'Требования',
  prerequisitesHint:
    'Проверяются при выдаче черты: лист предупредит о несоответствии, но взять черту не помешает.',
  /**
   * Тултип секции требований: категория черты D&D 2024 задаётся
   * требованиями, отдельного поля нет.
   */
  prerequisitesCategoriesHint:
    'Категорию черты (происхождения, общая, стиль боя, эпический дар) '
    + 'выражают требования: у черт происхождения их нет, общие требуют 4-го '
    + 'уровня, эпические дары — 19-го, а черты стиля боя — умения «Стиль '
    + 'боя» (его впишите произвольным требованием).',
...
export const FEAT_FORM_LABELS = {
  ...
  repeatableHint:
    ...
    + 'пропустит.',
  spellsHint:
    'Заклинания, которые черта выдаёт автоматически (всегда подготовлены). '
    + 'Совпавшее с компендиумом выдаётся при применении черты к персонажу.',
  effectsHint:
  ...
  /**
   * Вводный текст вкладки «Автоматизация» — что такое дары и когда они
   * применяются.
   */
  grantsHint:
    'Дары черты: владения, защиты, чувства, модификаторы листа и требования. '
    + 'Лист применяет их автоматически при взятии черты; требования только '
    + 'проверяются — с предупреждением, но без запрета.',
...
export const BACKGROUND_FORM_LABELS = {
  ...
  /** Раздел тот же, что показывает мастер, — подпись берётся оттуда же */
  equipmentTitle: BACKGROUND_WIZARD_LABELS.equipmentTitle,
  ...
  /**
   * Видимое пояснение секции снаряжения: варианты формы — только текст,
   * автовыдачи в инвентарь нет (позиции items форма не пишет).
   */
  equipmentHelp:
    'Оба варианта записываются текстом: мастер настройки покажет их игроку, '
    + 'но предметы и золото в инвентарь автоматически не положит — их '
    + 'добавляют вручную из справочника.',
  grantsHint:
