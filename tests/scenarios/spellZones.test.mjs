...
describe('[SZ08–SZ09] зона на сцене: копии, урон на ходу и статус от входа', () => {
  ...
  it('[SZ09] вход в зону: спасбросок у игрока, провал — «Опутанный» до исчезновения зоны', async () => {
    const zone = createSpellZone();
  ...
  it('статус от зоны мастера исчезновение зоны переживает', () => {
...
describe('числа источника в эффектах', () => {
