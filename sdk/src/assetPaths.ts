...
/**
 * Потолок числа пикселей входной картинки для форматов, которые
 * распаковываются в память ЦЕЛИКОМ (png, webp, gif, avif).
 *
 * Лимит по размеру файла от «пиксельной бомбы» не защищает: сильно сжатый PNG
 * в пару мегабайт разворачивается в гигабайты растра и кладёт сервер мира по
 * памяти от одной загрузки. 100 млн пикселей заметно больше предельной стороны
 * {@link MEDIA_MAX_DIMENSION_PX} (8192×8192 ≈ 67 млн), поэтому реальные карты
 * проходят, а неправдоподобные размеры отсекаются.
 */
export const IMAGE_MAX_INPUT_PIXELS = 100_000_000;

/**
 * Потолок числа пикселей входного JPEG.
 *
 * У JPEG потолок ВЫШЕ, и это не послабление, а разная цена разбора: libvips
 * гонит его строками, полный растр в памяти не появляется. Замер на шумной
 * картинке 16000×12000 (192 Мпикс, файл 130 МБ): ресайз до 8192 и перекодировка
 * в webp — 15 с при пике памяти +20 МБ. Упирается такой разбор во ВРЕМЯ, а не в
 * память, поэтому лимит выставлен с запасом под всё, что вообще влезает в
 * {@link MAX_FILE_SIZE}: экспорт карты 20000×20000 обязан загружаться, а не
 * молча оставаться исходником.
 */
export const JPEG_MAX_INPUT_PIXELS = 400_000_000;

/**
 * Возвращает потолок пикселей для формата картинки.
 *
 * Принимает и MIME (`image/jpeg` — так его знает клиент), и имя формата от
 * sharp (`jpeg` — так его знает сервер): правило одно на обе стороны, поэтому и
 * живёт оно в одном месте.
 *
 * @param format - MIME-тип или имя формата картинки
 * @returns предельное число пикселей для этого формата
 */
export function resolveImageInputPixelLimit(
...
/** Пикселей в мегапикселе — делитель для чисел в сообщениях пользователю */
const PIXELS_PER_MEGAPIXEL = 1_000_000;

/**
 * Собирает фразу отказа для картинки, не прошедшей по числу пикселей.
 *
 * Фраза одна на клиент (предпроверка перед отправкой) и сервер (гейт загрузки).
 * Разойдись они — на один и тот же отказ пользователь получал бы два разных
 * объяснения, смотря кто его поймал первым.
 *
 * @param limitPixels - потолок, действующий для формата картинки
 * @param dimensions - размеры исходника, если они уже известны
 * @param dimensions.width - ширина исходника в пикселях
 * @param dimensions.height - высота исходника в пикселях
 * @returns готовый текст для тоста
 */
export function describeImagePixelLimit(
  limitPixels: number,
  dimensions?: { readonly width: number; readonly height: number },
): string {
  const subject = dimensions
    ? `Изображение ${dimensions.width}×${dimensions.height}`
    : 'Изображение';

  const limitInMegapixels = Math.round(limitPixels / PIXELS_PER_MEGAPIXEL);

  return `${subject} слишком большое: предел — ${limitInMegapixels} млн пикселей. Уменьшите картинку и загрузите снова.`;
}
