...
/**
 * Дальность хода сущности за один ход — для подсветки зоны перемещения.
 *
...
export interface VttSystem {
   ...
   *
   * Ядро рисует по ней зону перемещения выделенного токена (удержание Alt).
   * Правило «насколько ускоряет разгон» системо-зависимо и живёт в системе —
   * ядро только переводит единицы дистанции в клетки сетки.
   *
   * Опционально: `undefined` (метод не реализован) означает, что система не
